...
| Decision | Rationale |
|---|---|
| ELT over ETL | Leverage PostgreSQL processing power for transformations |
...
\`\`\`bash
docker compose ps
docker compose logs airflow_runner
docker compose logs dw_postgres
\`\`\`
...
# If missing, restart — dbt installs on startup
...
# schema.yml must be inside models/ not the project root
\`\`\`
